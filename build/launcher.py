...
def selftest():
    ...
    # What is being packaged has to be a *deployment* configuration, not a
    # developer's. Checked here rather than trusted from build.ps1's environment,
    # because this is the process that actually runs the shipped code — and a
    # build that quietly shipped DEBUG=True would put a traceback with every
    # setting in it in front of anyone who could reach an error page.
    if django_settings.DEBUG:
        raise SystemExit('refusing to package: DEBUG is on in the shipped configuration')
    if not django_settings.SECRET_KEY:
        raise SystemExit('refusing to package: no DJANGO_SECRET_KEY')
    if (Path(django_settings.BASE_DIR) / '.secret_key').exists():
        # A key generated by a developer's checkout must never travel in the
        # payload: every installation would then sign with the same one.
        raise SystemExit('refusing to package: a generated .secret_key is in the payload')
